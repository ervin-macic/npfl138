...
class Tacotron(npfl138.TrainableModule):
    ...
    def forward(self, texts: torch.Tensor, spectrograms_len: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # TODO(tacotron): Finally, compute `mel_frames_post` by passing `mel_frames` through the post-net.
        mel_frames_post = ...

        return mel_frames_post, mel_frames, gates

    def compute_loss(
        self, y_pred: tuple[torch.Tensor, torch.Tensor, torch.Tensor], y_true: tuple[torch.Tensor, torch.Tensor],
