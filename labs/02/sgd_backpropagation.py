...
class Model(torch.nn.Module):
    ...
    def train_epoch(self, dataset: MNIST.Dataset) -> None:
        ...
        for batch in dataset.batches(self._args.batch_size, shuffle=True):
            ...
            # i know that logits shape is (n x 10) where n = len(batch["images"])
            labels = batch["labels"]
            labels = torch.nn.functional.one_hot(labels.long(), num_classes=10).float()
            # dim = 1 to eliminate the columns
            loss = (-labels * probabilities.log()).sum(dim=1).mean()

            # We create a list of all parameters. Note that a `torch.nn.Module` automatically
            # tracks its parameters, so we could also use `list(self.parameters())`. good to know
    ...
    def evaluate(self, dataset: MNIST.Dataset) -> float:
        ...
        with torch.no_grad():
            ...
            for batch in dataset.batches(self._args.batch_size):
                ...
                correct += (logits.argmax(dim=1) == batch["labels"]).sum()
