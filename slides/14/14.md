...
Finally, in the architectures discussed so far, the current state in the
attention $→s_{i-1}$ has been represented using the decoder cell state.

~~~
However, in this model, it is beneficial for the attention to have access not
just to the decoder cell state but also to the last spectrogram frame processed
by the pre-net (which is on the input of the decoder in the next step and
therefore in the _next_ decoder cell state, one time step too late).

~~~
Therefore, a separate _attention RNN_ is introduced to track both the
spectrogram frames and context vectors generated so far.

~~~
- Its inputs are the generated pre-net-processed frames concatenated with the
  attention context vectors, and the output of this attention RNN is used as
  $→s_{i-1}$ in the attention computation (that is the sole purpose of this
  attention RNN).
